...
void Serialize(nlohmann::json& j, Model& model)
{
    ...
    j["pbr_enabled"] = model.GetPBREnabled();
    j["compressed"] = model.GetCompressed();
...
void DeserializeMap(const std::string& path)
{
    ...
    for(auto& model_json : j["models"])
    {
        if(model_json["name"].get<std::string>().size() > 0){
            ...
            model_id = LoadModel(model_json["path"], model_json["compressed"], model_json["pbr_enabled"], model_json["gamma_correction"]);
        }
